...
| **Execution Accuracy (EX)** | **74.7%** | 在真实 .sqlite 数据库上执行结果一致率（1034 条全量评估） |
| Exact Match (EM) | 44.0% | SQL 文本归一化后精确匹配率 |
| SQL 生成有效率 | 95.5% | 仅 4.5% 生成的 SQL 执行报错 |
...
1. **发现空表评估虚高问题**：之前的 EX=87.5% 是在从 schema DDL 构建的空表上评估，空表上逻辑错误的 SQL 因没有数据碰巧返回相同空集。在真实 .sqlite 数据库上评估后，EX 从 87.5% 修正为 74.7%，这才是与学术论文可比的真实指标
2. **EM 与 EX 存在系统性差距**：EM=44.0% vs EX=74.7%，差距 30.7 个百分点。EM 衡量的是"写法记忆"，EX 衡量的是"执行正确性"。学术界逐步转向以 EX 作为主评估指标（BIRD 已完全弃用 EM）
3. **高效微调范式**：QLoRA 4-bit 量化，单卡 16GB 显存完成 7B 模型微调，Adapter 仅 308MB
4. **完整端到端流程**：覆盖数据集构建 -> 数据增强 -> QLoRA 训练 -> 真实数据库 EX 评估 -> Web Demo 部署全流程
5. **pred_failed 仅 4.5%**：在真实数据库上，模型生成的 SQL 95.5% 可正确执行，主要失败原因是列名幻觉（引用不存在的 T2.pet_type 等）
...
| Web Demo | Gradio | - |
| 评估执行 | SQLite3（真实 .sqlite 数据库文件） | Python 内置 |
...
### 6.2 EM vs EX 差距的根本原因

### 6.2.1 空表评估 vs 真实数据库评估（重要修正）

初版评估使用从 schema DDL 构建的空 in-memory SQLite 表，导致 EX 虚高。修正后使用 Spider 提供的真实 .sqlite 数据库（含完整数据），结果如下：

| 评估方式 | EM | EX | EM-EX差距 | 样本量 |
|----------|-----|-----|-----------|--------|
| 空表评估（初版） | 37.5% | 87.5% | 50.0% | 200 |
| **真实数据库（修正）** | **44.0%** | **74.7%** | **30.7%** | **1034** |

**空表虚高的根本原因**：

空表上两条逻辑不同的 SQL 可能返回相同的空结果集（因为表中没有数据），例如：
```sql
-- Gold SQL: SELECT name FROM employees WHERE salary > 50000
-- Pred SQL: SELECT name FROM employees WHERE salary > 40000
-- 空表上：两条都返回空集 → EX 判定正确 ✅（错误判定）
-- 真实数据库上：返回不同结果 → EX 判定错误 ❌（正确判定）
```

### 6.2.2 三轮实验空表评估对比（仅作参考）

三轮实验空表评估均显示 EM 比 EX 低 40-50 个百分点：
...
| 失败类型 | V1 | V2 | V3 | 说明 |
|----------|-----|-----|-----|------|
| pred_failed（SQL 执行失败） | 11.5% | 10.5% | 17.0% | 模型生成的 SQL 在 SQLite 上无法执行 |
| result_mismatch（执行结果不同） | 1.5% | 2.0% | 1.5% | 逻辑上确实有差异 |
| exact_match（EX=1） | **87.0%** | **87.5%** | 81.5% | 执行结果完全一致 |

**关键结论**：
1. **空表评估导致 EX 虚高**，真实数据库 EX=74.7% 才是与学术论文可比的指标
2. **EM 与 EX 的真实差距为 30.7%**（空表评估时被夸大为 50%）
3. **pred_failed 仅 4.5%**（真实数据库上），比空表的 10.5% 更低——模型语法正确率很高
4. **result_mismatch 占 20.8%**（真实数据库上），远高于空表的 2.0%——这才是真正的逻辑错误，空表掩盖了这些问题
...
| 方法 | 数据集 | 评估方式 | EX |
|------|--------|---------|-----|
| 本项目 (V2) | Spider dev | 真实 .sqlite 数据库 | **74.7%** |
| DIN-SQL | Spider dev | 真实数据库 | 82.4% |
| RESDSQL | Spider dev | 真实数据库 | 79.9% |

> 本项目 EX 低于 DIN-SQL 等方法，原因是仅使用了 SFT 微调 + 7B 模型 + 单轮推理，而 DIN-SQL 使用了 GPT-4 + 专门的 schema linking + prompt engineering。在相同方法级别（SFT 微调开源模型）中，74.7% 是合理的基线表现。

---

## 8. 失败模式深度分析

### 8.1 失败分布（V2, 真实数据库, 1034 条全量评估）

```
exact_match (EX成功): ████████████████████████████  74.7%  (772/1034)
result_mismatch:      ████████                         20.8%  (216/1034)
pred_failed:          ▌                                 4.5%   (46/1034)
```

### 8.2 pred_failed 根因分析（46 条，真实数据库）
...
| 错误类型 | 占比 | 示例 |
|---------|------|------|
| 列名幻觉 | ~60% | 引用不存在的 T2.pet_type、T2.Weight 等列 |
| 表名幻觉 | ~10% | 引用不存在的 degrees_programs 表 |
| 聚合函数误用 | ~5% | misuse of aggregate function count() |
| 编码问题 | ~5% | Could not decode to UTF-8 column |
| 其他语法错误 | ~20% | 括号缺失、大小写不一致等 |

**修复方向**：
1. Schema 信息增强：在 prompt 中加入更多列名细节，减少列名幻觉
2. SQL 语法归一化后处理
3. 自一致性推理（多候选中选最稳定的）
...
│   │   ├── ...
│   │   └── final/
│   ├── execution_accuracy_real_db_report.json  # 真实数据库 EX 评估报告
...
#### 判断 1：空表评估虚高，真实数据库才是标准

初版在空 schema 表上 EX=87.5%，真实数据库上 EX=74.7%，差距 12.8%。空表因没有数据，逻辑错误的 SQL 碰巧返回相同空集。**EX 必须在真实数据库上评估才能与学术论文对比。**

#### 判断 2：EM 与 EX 存在系统性差距，但不如空表评估时夸张

真实数据库上 EM=44.0% vs EX=74.7%，差距 30.7%（空表评估时显示 50%）。EM 衡量写法记忆，EX 衡量执行正确性，差距始终存在但空表评估夸大了它。**以 EX 在真实数据库上的结果做决策。**

#### 判断 3：eval_loss 和 EM 可能反向

V3 去掉 MLP 层（gate/up/down）导致 medium 难度从 27.3% 暴跌到 11.4%。SwiGLU 架构的 FFN 层负责复杂的数据库关系推理，去掉后模型失去了处理 JOIN 等复杂操作的能力。

#### 判断 4：MLP 层对复杂 SQL 至关重要
...
### 12.1 当前基线（V2 final，真实数据库评估）

- **EX: 74.7%**（真实 .sqlite 数据库，1034 条全量）
- EM: 44.0%
- pred_failed: 4.5%（46 条，主要为列名幻觉）
- result_mismatch: 20.8%（216 条，逻辑差异）
...
预期效果：pred_failed 从 10.5% 降至 ~5%，EX 提升 3-5%
...
#### 方案 1.3：使用真实 .sqlite 数据库文件

当前使用 schema DDL 构建空 in-memory 表，无法真正存储数据。自一致性推理依赖真实数据分布。
...
| 阶段 | 目标 EX | 手段 |
|------|---------|------|
| 当前基线 | 74.7% | V2 checkpoint + 真实数据库评估 |
| Phase 1 Schema 增强 + 后处理 | 78-80% | 减少 pred_failed（列名幻觉） |
| Phase 1 自一致性推理 | 80-82% | 推理增强（无需重训练） |
| Phase 2 最优配置重训 | 82-85% | 最优训练配置 |
...
| V2 | 2026-05 | rank=32, early stop，空表 EX=87.5%，真实数据库 EX=74.7% |
| V3 | 2026-05 | rank=16, 去掉 MLP，效果下降 |
| Final | 2026-05 | 确定 V2 为生产模型，真实数据库 EX=74.7% |
