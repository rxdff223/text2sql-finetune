...
| **Execution Accuracy (EX)** | **87.5%** | 在 SQLite 上执行结果一致率 |
...
**关键结论**：
1. **pred_failed 是主要损失**（10.5-17%）
2. **result_mismatch 只有 1.5-2.0%** — 模型生成的 SQL 逻辑几乎全部正确
3. **绝大多数"EM 失败"只是格式差异**，执行结果完全正确
...
| hard 难度 EX | 76.7% | **86.7%** | 73.3% |
| medium 难度 EX | 86.5% | 85.4% | 73.0% |

**选择 V2 的理由**：
1. EX 最高（87.5%）
2. Adapter 仅 308MB，推理速度快
3. eval_loss 最稳定，训练过程健康
4. **hard 难度反而领先 V1**（86.7% vs 76.7%）

### 7.2 按难度分层 EX（V2 final）

| 难度 | EX | 样本数 | 表现 |
|------|-----|--------|------|
| easy | 90.9% | - | 简单查询几乎全对 |
| medium | 85.4% | - | 中等复杂度稳定 |
| hard | 86.7% | - | 复杂 JOIN 反而更强 |
| extra_hard | 75.0% | - | 样本少，波动大 |

### 7.3 与学术界基准对比

| 方法 | 数据集 | 指标 | 数值 |
|------|--------|------|------|
| 本项目 (V2) | Spider dev | EX | **87.5%** |
| DIN-SQL | Spider dev | EX | 82.4% |
| RESDSQL | Spider dev | EX | 79.9% |
| SMBA-SOTA | Spider dev | EX | 78.9% |
| PICARD | Spider dev | EX | 75.5% |

> 注：不同方法的评估协议可能存在差异（SQLite vs 原生 DB），数字仅供参考

---

## 8. 失败模式深度分析

### 8.1 失败分布（V2, 200 条评估样本）

```
exact_match (成功):   ████████████████████████████████████  87.5%  (175/200)
pred_failed (失败):  ████                                     10.5%  (21/200)
result_mismatch:     ▌                                        2.0%   (4/200)
```
...
│   ├── evaluate.py               # EM 评估脚本
│   ├── execution_eval.py         # EX 评估引擎（自研 in-memory SQLite）
...
- **EX: 87.5%**（生产模型）
- pred_failed: 10.5%（语法兼容性）
- result_mismatch: 2.0%（逻辑错误）
...
| 阶段 | 目标 EX | 手段 |
|------|---------|------|
| 当前基线 | 87.5% | V2 checkpoint |
| Phase 1 语法归一化 | 90-92% | 后处理（无需重训练） |
| Phase 1 自一致性 | 92-94% | 推理增强（无需重训练） |
| Phase 2 最优配置重训 | 93-95% | 最优训练配置 |
